...
const envSchema = z.object({
  ...
  OPENAI_API_KEY: z.string().min(1).optional().default("not-configured"),
  OPENAI_TTS_MODEL: z.string().min(1).default("gpt-4o-mini-tts"),
  ELEVENLABS_API_KEY: z.string().min(1).optional().default("not-configured"),
  ELEVENLABS_VOICE_ID: z.string().min(1).default("Rachel"),
  CARTESIA_API_KEY: z.string().min(1).optional().default("not-configured"),
  CARTESIA_VOICE_ID: z.string().min(1).default("default"),
  TTS_PROVIDER: z.enum(["openai", "elevenlabs", "cartesia"]).default("openai"),
